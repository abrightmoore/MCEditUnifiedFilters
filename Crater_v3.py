# This filter is for generating a Crater
...
def perform(level,box, options):
	...
	crater(levelCopy,boxCopy,options)
...
def crater(level,box,options):
	...
	# A crater has a bump in the middle, a basin with layers of material, subsidence rings, and a ring with ejecta lines.
	
	# Step 1 is to vaporise the material in the selection box in a hemisphere down to the middle of the box
	THRESHHOLD = 0.4
	a = centreWidth
	b = centreHeight
	c = centreDepth
	for (x,y,z) in box.positions:
		dx = (centreWidth-x)/1
		dy = (centreHeight-y) #(height-1-y)/1
		dz = (centreDepth-z)/1
		wterm = (dx**2)/(a**2)
		hterm = (dy**2)/(b**2)
		dterm = (dz**2)/(c**2)
		value = float(wterm + hterm + dterm)
		# rings of material
		dh = (wterm + dterm)
		if value <= THRESHHOLD: # or dy > 0:
			setBlockForced(level, AIR, box.minx+centreWidth+dx,box.miny+centreHeight+dy,box.minz+centreDepth+dz)
		if dy > centreHeight/5:
			ddy = 0
		else:
			ddy = (centreHeight/5-dy)*0.03
		if dy > 0 and dy < centreHeight/5 and value > THRESHHOLD and value < THRESHHOLD+0.2+ddy: #THRESHHOLD+centreHeight/5-dy/5*0.01:
			drawLineForced(level,(1,0), (box.minx+centreWidth+dx,box.miny+centreHeight+dy,box.minz+centreDepth+dz),(box.minx+centreWidth+dx,box.miny,box.minz))
			# setBlockToGroundForced(level, (1,0), box.minx+centreWidth+dx,box.miny+centreHeight+dy,box.minz+centreDepth+dz)
#		elif value-1 < 0.5 and dy > 1:
#			setBlockForced(level, (1,0), box.minx+centreWidth+dx,box.maxy-1-dy-dh,box.minz+centreDepth+dz)
	# Store the seed on a sign
	(x,y,z) = (cw, ch, cd)
	setBlock(level, (1,0), box.minx+x,box.miny+y,box.minz+z)
	createSign(level, box.minx+x, box.miny+y+1, box.minz+z, str(SEED))
...
def setBlockToGroundForced(level, (block, data), x, y, z):
	dy = y
	while dy >= 0:
		block = getBlock(level, x, dy, z)
		if block == 0:
			setBlockForced(level, (block, data), x, dy, z)
		else:
			dy = -1 # end
		dy = dy-1
