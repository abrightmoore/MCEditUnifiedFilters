...
#				if dGap < matchGap: # Clear match - closer
#					matched = ((matID, matData), dAngles, dGap)
...
#			else: # Colour is more different, so length better be very close
#				if  abs(dAngles)*dGap < abs(matchAngle)*matchGap:
#					matched = ((matID, matData), dAngles, dGap)
...
def closestMaterial2((r, g, b, a)):
	matched = ((35,15),pi,512)
	for m in materials:
		(isBuildBlock, textureFileName, (matID,matData), mr, mg, mb, mavg, mpr, mpg, mpb) = m # unpack
		if isBuildBlock == 1: # This block is a candidate.
			(theta, phi) = getColourProximity((r,g,b),(mr,mg,mb))
			dAngles = abs(theta)+abs(phi) # Summarise the gap in angles
			dRed = r-mr
			dGreen = g-mg
			dBlue = b-mb
			dGap = sqrt(dRed**2+dGreen**2+dBlue**2)
			
			(matchMat,matchAngle,matchGap) = matched
...
def closestMaterial1((r, g, b, a)): # Original concept by @Sethbling. This implementation @abrightmoore
	''' Deprecated
	'''
	# print r,g,b,a
	# print r,g,b,a
	closest = 512*512*3
	best = (35, 0, 512, 512, 512, 512)
	...
	for m in materials:
		(isBuildBlock, textureFileName, (matID,matData), mr, mg, mb, mavg, mpr, mpg, mpb) = m # unpack
		if isBuildBlock == 1: # This block is a candidate.
			greyMatch = avg-mavg # How close are we on greyscale

			distRed = r-mr
			distGreen = g-mg
			distBlue = b-mb
			
			# How close are the colours when the intensity is equalised?
			colourStdRed = r-(mr+greyMatch)
			colourStdGreen = g-(mg+greyMatch)
			colourStdBlue = b-(mb+greyMatch)
			
			# Now... sort the results by how well they match.
			
			# Is this a direct match?
			if mr == r and mg == g and mb == b:
				return (matID,matData) # Straight up matched
			else:
				if len(results) == 0: # Only case is best case
					results.append(((matID,matData),greyMatch,distRed,distGreen,distBlue,colourStdRed,colourStdGreen,colourStdBlue))
				else: # There is an existing list. Work out whether to add before or after the top element
					n = results[0]
					(nMat,nGreyMatch,nDistRed,nDistGreen,nDistBlue,nColourStdRed,nColourStdGreen,nColourStdBlue) = n
					# If the greyMatch colourstd match is better than the existing one, prepend
					mColDifR = abs(distRed)-abs(nDistRed)
					mColDifG = abs(distGreen)-abs(nDistGreen)
					mColDifB = abs(distBlue)-abs(nDistBlue)
					maxColDif = mColDifR+mColDifG+mColDifB
					if maxColDif > 0 and abs(greyMatch) > abs(nGreyMatch): # On average this is a worse match
						results.append(((matID,matData),greyMatch,distRed,distGreen,distBlue,colourStdRed,colourStdGreen,colourStdBlue))
					elif maxColDif > 0:
						results.append(((matID,matData),greyMatch,distRed,distGreen,distBlue,colourStdRed,colourStdGreen,colourStdBlue))
					else:
						results.insert(0,((matID,matData),greyMatch,distRed,distGreen,distBlue,colourStdRed,colourStdGreen,colourStdBlue)) # This may be a better match
					
	if len(results) > 0:
		(Mat,gm,dr,dg,db,csr,csg,csb) = results[0]
		return Mat # Choicest block match
	else:
		return (89,0) # No match - mark Glowstone
...
def writeOutStairs(level,box,options):
	# Carve a set of stair rotations from this block and save them as files with suffixes
	b=range(4096)
	b.remove(0) # @CodeWarrior0 and @Wout12345 explained how to merge schematics			

		
	path = options["Path and Filename"]

	width = box.maxx-box.minx
	depth = box.maxz-box.minz
	height = box.maxy-box.miny

	# Copy the selection box to keep it safe
	SourceBox = BoundingBox((box.minx,box.miny,box.minz),(width,height,depth))
	SourceSchema = level.extractSchematic(SourceBox)
	fname = path
	if not fname.endswith(("\\","/")):
		fname += "/"
	SourceSchema.saveToFile(path+str(options["Block ID"])+"_"+str(options["Block Data"])+".schematic")
	# The Stair shape
	stairSize = (width,height,depth)
	stairSchematic = MCSchematic(shape=stairSize)
	stairSchematic.copyBlocksFrom(SourceSchema, BoundingBox((0,0,0),(stairSize)), (0,0,0)) #SourceSchema.extractSchematic(SourceBox) # 
	stairBox = BoundingBox((0,0,0),(stairSize)) # BoundingBox(stairSize)	

	
	# Lower
	# E _0
	
	# Copy the 1st face to inside
	faceBox = BoundingBox((0,(height>>1),1),(1,(height>>1)-1,depth-2))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(width>>1,(height>>1),1))
	# Copy the 2nd face to inside
	faceBox = BoundingBox((1,height-1,1),((width>>1)-1,1,depth-1))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(1,(height>>1)-1,1))
	# Clear out the air-ea
	shapeFill(stairSchematic,(0,0),(0,(height>>1),0),((width>>1)-1,height-1,depth))
	fname = path
	if not fname.endswith(("\\","/")):
		fname += "/"
	fname += str(options["Block ID"])+"_"+str(options["Block Data"])
	fname += "_stair_"
	fname += "0_east"
	fname += ".schematic"
	stairSchematic.saveToFile(fname)
	

	# W _1
	stairSchematic.copyBlocksFrom(SourceSchema, BoundingBox((0,0,0),(stairSize)), (0,0,0)) # init
	faceBox = BoundingBox((width-1,(height>>1),1),(1,(height>>1)-1,depth-2))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),((width>>1)-1,(height>>1),1))
	# Copy the 2nd face to inside
	faceBox = BoundingBox((width>>1,height-1,1),((width>>1)-1,1,depth-1))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(width>>1,(height>>1)-1,1))
	# Clear out the air-ea
	shapeFill(stairSchematic,(0,0),(width>>1,(height>>1),0),(width-1,height-1,depth))
	fname = path
	if not fname.endswith(("\\","/")):
		fname += "/"
	fname += str(options["Block ID"])+"_"+str(options["Block Data"])
	...
	# S _2
	stairSchematic.copyBlocksFrom(SourceSchema, BoundingBox((0,0,0),(stairSize)), (0,0,0)) # init
	faceBox = BoundingBox((1,(height>>1),0),(width-1,(height>>1)-1,1))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(1,(height>>1),(depth>>1)))
	# Copy the 2nd face to inside
	faceBox = BoundingBox((1,height-1,1),(width-1,1,(depth>>1)-1))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(1,(height>>1)-1,1))
	# Clear out the air-ea
	shapeFill(stairSchematic,(0,0),(0,(height>>1),0),(width-1,height-1,(depth>>1)-1))
	fname = path
	if not fname.endswith(("\\","/")):
		fname += "/"
	fname += str(options["Block ID"])+"_"+str(options["Block Data"])
	...
	# N _3
	stairSchematic.copyBlocksFrom(SourceSchema, BoundingBox((0,0,0),(stairSize)), (0,0,0)) # init
	faceBox = BoundingBox((1,(height>>1),0),(width-1,(height>>1)-1,depth-1))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(1,(height>>1),(depth>>1)-1))
	# Copy the 2nd face to inside
	faceBox = BoundingBox((1,height-1,depth>>1),(width-2,1,(depth>>1)-1))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(1,(height>>1)-1,depth>>1))
	# Clear out the air-ea
	shapeFill(stairSchematic,(0,0),(0,(height>>1),depth>>1),(width-1,height-1,depth-1))
	fname = path
	if not fname.endswith(("\\","/")):
		fname += "/"
	fname += str(options["Block ID"])+"_"+str(options["Block Data"])
	...
	stairSchematic.copyBlocksFrom(SourceSchema, BoundingBox((0,0,0),(stairSize)), (0,0,0)) # init
	# Copy the 1st face to inside
	faceBox = BoundingBox((0,1,1),(1,(height>>1)-1,depth-2))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(width>>1,1,1))
	# Copy the 2nd face to inside
	faceBox = BoundingBox((1,0,1),((width>>1)-1,1,depth-1))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(1,(height>>1),1))
	# Clear out the air-ea
	shapeFill(stairSchematic,(0,0),(0,0,0),((width>>1)-1,(height>>1)-1,depth))
	fname = path
	if not fname.endswith(("\\","/")):
		fname += "/"
	fname += str(options["Block ID"])+"_"+str(options["Block Data"])
	...
	# W _1
	stairSchematic.copyBlocksFrom(SourceSchema, BoundingBox((0,0,0),(stairSize)), (0,0,0)) # init
	faceBox = BoundingBox((width-1,1,1),(1,(height>>1)-1,depth-2))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),((width>>1)-1,1,1))
	# Copy the 2nd face to inside
	faceBox = BoundingBox((width>>1,0,1),((width>>1)-1,1,depth-1))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(width>>1,(height>>1),1))
	# Clear out the air-ea
	shapeFill(stairSchematic,(0,0),(width>>1,0,0),(width-1,(height>>1)-1,depth))
	fname = path
	if not fname.endswith(("\\","/")):
		fname += "/"
	fname += str(options["Block ID"])+"_"+str(options["Block Data"])
	...
	# S _2
	stairSchematic.copyBlocksFrom(SourceSchema, BoundingBox((0,0,0),(stairSize)), (0,0,0)) # init
	faceBox = BoundingBox((1,1,0),(width-1,(height>>1)-1,1))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(1,1,(depth>>1)))
	# Copy the 2nd face to inside
	faceBox = BoundingBox((1,0,1),(width-1,1,(depth>>1)-1))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(1,(height>>1),1))
	# Clear out the air-ea
	shapeFill(stairSchematic,(0,0),(0,0,0),(width-1,(height>>1)-1,(depth>>1)-1))
	fname = path
	if not fname.endswith(("\\","/")):
		fname += "/"
	fname += str(options["Block ID"])+"_"+str(options["Block Data"])
	...
	# N _3
	stairSchematic.copyBlocksFrom(SourceSchema, BoundingBox((0,0,0),(stairSize)), (0,0,0)) # init
	faceBox = BoundingBox((1,1,0),(width-1,(height>>1)-1,depth-1))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(1,1,(depth>>1)-1))
	# Copy the 2nd face to inside
	faceBox = BoundingBox((1,height-1,depth>>1),(width-2,1,(depth>>1)-1))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(1,(height>>1),depth>>1))
	# Clear out the air-ea
	shapeFill(stairSchematic,(0,0),(0,0,depth>>1),(width-1,(height>>1)-1,depth-1))
	fname = path
	if not fname.endswith(("\\","/")):
		fname += "/"
	fname += str(options["Block ID"])+"_"+str(options["Block Data"])
	...
	# Upper slab
	stairSchematic.copyBlocksFrom(SourceSchema, BoundingBox((0,0,0),(stairSize)), (0,0,0)) # init
	faceBox = BoundingBox((0,0,0),(width,1,depth))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(0,height>>1,0))
	# Clear out the air-ea
	shapeFill(stairSchematic,(0,0),(0,0,0),(width,(height>>1)-1,depth))
	fname = path
	if not fname.endswith(("\\","/")):
		fname += "/"
	fname += str(options["Block ID"])+"_"+str(options["Block Data"])
	...
	faceBox = BoundingBox((0,0,0),(width,1,depth))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(0,(height>>1)-1,0))
	# Clear out the air-ea
	shapeFill(stairSchematic,(0,0),(0,height>>1,0),(width,height-1,depth))
	fname = path
	if not fname.endswith(("\\","/")):
		fname += "/"
	fname += str(options["Block ID"])+"_"+str(options["Block Data"])
	...
	# Fence post
	shapeFill(stairSchematic,(0,0),(0,0,0),(stairSchematic.Width,stairSchematic.Height,stairSchematic.Length)) # init
	faceBox = BoundingBox((6,0,0),(4,height,2))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(6,0,6))

	faceBox = BoundingBox((6,0,depth-2),(4,height,2))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(6,0,8))

	faceBox = BoundingBox((0,0,6),(1,height,4))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(6,0,6))

	faceBox = BoundingBox((width-1,0,6),(1,height,4))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(9,0,6))
	...
	# EW
	shapeFill(stairSchematic,(0,0),(0,0,0),(stairSchematic.Width,stairSchematic.Height,stairSchematic.Length)) # init
	faceBox = BoundingBox((0,0,0),(width,height,1))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(0,0,7))

	faceBox = BoundingBox((0,0,depth-1),(width,height,1))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(0,0,8))
	# We have a wall here - Save it for funsies (TBD)
	
	shapeFill(stairSchematic,(0,0),(2,0,7),(width-3,0,8))
	shapeFill(stairSchematic,(0,0),(2,4,7),(5,6,8))
	shapeFill(stairSchematic,(0,0),(10,4,7),(width-3,6,8))
	shapeFill(stairSchematic,(0,0),(0,11,7),(width-1,height-1,8))
	shapeFill(stairSchematic,(0,0),(2,10,7),(width-3,10,8))
	...
	# SN
	shapeFill(stairSchematic,(0,0),(0,0,0),(stairSchematic.Width,stairSchematic.Height,stairSchematic.Length)) # init
	faceBox = BoundingBox((0,0,0),(1,height,depth))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(7,0,0))

	faceBox = BoundingBox((0,0,0),(1,height,depth))
	faceSchema = SourceSchema.extractSchematic(faceBox)
	stairSchematic.copyBlocksFrom(faceSchema, BoundingBox((0,0,0),(faceSchema.Width,faceSchema.Height,faceSchema.Length)),(8,0,0))
	# We have a wall here - Save it for funsies (TBD)
	
	shapeFill(stairSchematic,(0,0),(7,0,2),(8,0,depth-3))
	shapeFill(stairSchematic,(0,0),(7,4,2),(8,6,5))
	shapeFill(stairSchematic,(0,0),(7,4,10),(8,6,depth-3))
	shapeFill(stairSchematic,(0,0),(7,11,0),(8,height-1,depth-1))
	shapeFill(stairSchematic,(0,0),(7,10,2),(8,10,depth-3))
...
def perform(level, box, options):
	...
	if options["Block ID"] < 1:
		cubePNG(level, box, options, options["Path and Filename"])	
	else:
		blockCube(level,box,options)
	
	if options["Write out stairs?"] == True:
		writeOutStairs(level,box,options)
	
	level.markDirtyBox(box)

def blockCube(level,box,options):
	# Given a block ID, build it big!
	
	blockID = options["Block ID"]
	blockData = options["Block Data"]
	
	# Find all the textures
	definitions = []
	for (isBuildBlock, textureFileName, (matID,matData), mr, mg, mb, mavg, mpr, mpg, mpb) in materials:
		if (matID,matData) == (blockID,blockData):
			definitions.append((isBuildBlock, textureFileName, (matID,matData), mr, mg, mb, mavg, mpr, mpg, mpb))
	
	if len(definitions) > 0:
		(isBuildBlock, textureFileName, (matID,matData), mr, mg, mb, mavg, mpr, mpg, mpb) = definitions[0]
		cubePNG(level, box, options, options["Path and Filename"]+textureFileName)

def cubePNG(level, box, options, pathAndFileName): 
				...
				if RELIEF == False:
					if DIAGONAL == False:
						...
						if FLAT == True:
							...
							if SIDES == True:
								setBlock(level, (theBlock, theBlockData), box.minx+height-1-x, box.miny+height-y-1, box.minz) # front face
								setBlock(level, (theBlock, theBlockData), box.minx+x, box.miny+height-y-1, box.minz+height-1) # back face
								if AFO == True:
									setBlock(level, (theBlock, theBlockData), box.minx, box.miny+y, box.minz+height-1-x) # Left face
									...
									setBlock(level, (theBlock, theBlockData), box.minx+height-1, box.miny+height-y-1, box.minz+height-1-x) # right face
								if options["Fill?"] == True:
									materialFill = (options["Fill material"].ID, options["Fill material"].blockData)
									for x1 in xrange(box.minx+1,box.minx+15):
										for z1 in xrange(box.minz+1,box.minz+15):
											for y1 in xrange(box.miny+1,box.miny+15):
												setBlock(level,materialFill,x1,y1,z1)

					else: # diagonal
						setBlock(level, (theBlock, theBlockData), box.minx+x, box.miny+height-y-1, box.minz+x)
						setBlock(level, (theBlock, theBlockData), box.maxx-x-1, box.miny+height-y-1, box.minz+x)
