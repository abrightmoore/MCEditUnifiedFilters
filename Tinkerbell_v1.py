# This filter generates the Tinkerbell chaotic map.
...
def perform(level, box, options):
	...
	Tinkerbell(level, box, options)		
	level.markDirtyBox(box)

def Tinkerbell(level, box, options):
	# CONSTANTS
	method = "TINKERBELL ATTRACTOR"
	print '%s: Started at %s' % (method, time.ctime())
...
#		if STEPNUM %100 == 0:
		...
		dx = x * x - y * y + a * x + b * y
		dy = 2 * x * y + c * x + d * y
		setBlock(level, (BLOCKID, coloursList[colour]), (int)(box.minx+centreWidth+x*HORIZMAGSCALE), (int)(box.miny+centreHeight+z*VERTMAGSCALE), (int)(box.minz+centreDepth+y*HORIZMAGSCALE))
		
#		drawLine(level,(BLOCKID, coloursList[colour]),
#				 (box.minx+centreWidth+x*HORIZMAGSCALE, box.miny+centreHeight+z*VERTMAGSCALE, box.minz+centreDepth+y*HORIZMAGSCALE),
#				 (box.minx+centreWidth+dx*HORIZMAGSCALE, box.miny+centreHeight+dz*VERTMAGSCALE, box.minz+centreDepth+dy*HORIZMAGSCALE)
#				)
		x = dx
		y = dy
		z = dz
