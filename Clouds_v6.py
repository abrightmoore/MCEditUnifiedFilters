# This filter is for generating clouds.
...
def perform(level,box, options):
	...
	# The shape of the cloud is: flat bottom and billowing sections up to the top of the box.
	# Move through the layers, packing in spheres at irregular positions
	P = []
	delta = int((centreWidth+centreDepth)/2)
	an = 2*pi/delta
	dist = int((centreWidth+centreDepth)/8)
	R = []
	S = []
	(dx,dz) = (0,0)
	for i in xrange(0,height):
		...
		dd = 0.0
		if i > centreHeight:
			dd = float(float(i-centreHeight)/float(centreHeight-1))
		scalar = cos( dd*pi/2 )
		print scalar
		
		for j in xrange(0,delta):
			d = scalar*(RAND.random()*dist+int(delta/2))
			Q.append((boxCopy.minx+centreWidth+dx+d*cos(j*an),boxCopy.miny+i,boxCopy.minz+centreDepth+dz+d*sin(j*an)))
		Q.append(Q[0])
		Q.append(Q[1])
		P.append(Q)
		#drawLinesSmooth(levelCopy,boxCopy,MATERIAL,5,Q)
		R.append(plotLinesSmooth(levelCopy,boxCopy,MATERIAL,3,Q))
		(dx,dz) = (dx+RAND.randint(-1,2)*0.2,dz+RAND.randint(-1,2)*0.2)
	...
	for j in xrange(0,len(R[0])):
		...
		for i in xrange(0,r):
			...
			for t in T:
				S.append(t)
		drawLinesSmooth(levelCopy,boxCopy,MATERIAL,4,S)
...
def scatterSpheres(level,box,(x,y,z),r,MATERIAL,RAND,ratio):
	if itersFun < 1000 and int(r) > 0 and x-r >= box.minx and x+r < box.maxx and y+r < box.maxy and z-r >= box.minz and z+r < box.maxz:
		if randint(1,100) > 99:
			print (x,y,z,r)
		drawSphere(level,(x,y,z),int(r*ratio),MATERIAL)
		if RAND.randint(1,100) > 15:
			keepGoing = True
			counter = 0
			while keepGoing == True:
				counter = counter +1
				theta = RAND.random()*pi*2
				dy = RAND.randint(0,8)
				scatterSpheres(level,box,(x+(r)*cos(theta),y+dy,z+(r)*sin(theta)),r*0.7,MATERIAL,RAND,ratio)
				if counter > 10 or RAND.randint(0,99) < (95*(1-ratio)+1):
					keepGoing = False
...
def plotLine(scratchpad, (blockID, blockData), (x,y,z), (x1,y1,z1) ):
	return plotLineConstrained(scratchpad, (blockID, blockData), (x,y,z), (x1,y1,z1), 0 )
...
def plotLinesSmooth(level,box,material,SMOOTHAMOUNT,P):
	Q = []
	for i in xrange(0,SMOOTHAMOUNT):
		P = chaikinSmoothAlgorithm(P)
	Q = plotLines(level,box,material,P)
	return Q
...
def plotLines(level,box,material,P):
	Q = []
	count = 0
	(x0,y0,z0) = (0,0,0)
	for (x,y,z) in P:
		if count > 0:
			Q.append( plotLine(level,material,(box.minx+x0,box.miny+y0,box.minz+z0),(box.minx+x,box.miny+y,box.minz+z)) )
		count = count+1
		(x0,y0,z0) = (x,y,z)
	return Q
...
def plotLineConstrained(scratchpad, (blockID, blockData), (x,y,z), (x1,y1,z1), maxLength ):
	dx = x1 - x
	dy = y1 - y
	dz = z1 - z
