...
def drawLineConstrained(scratchpad, (blockID, blockData), (x,y,z), (x1,y1,z1), maxLength ):
	...
	if distance < maxLength or maxLength < 1:
		...
		while iter <= distance:
			...
			# print px,py,pz
			b = scratchpad.blockAt(int(px), int(py), int(pz))
			if b == 0:
				# print px, py, pz, blockID
				# print scratchpad
				scratchpad.setBlockAt(int(px), int(py), int(pz), blockID)
				scratchpad.setBlockDataAt(int(px), int(py), int(pz), blockData)
