# This filter replaces note blocks in the selection area with an equivalent command block using 'playsound'
# This filter: abrightmoore@yahoo.com.au (http://brightmoore.net)

# References:
# http://minecraft.gamepedia.com/Commands <- Playsound format
# http://minecraft.gamepedia.com/Note_Block <- Note Blocks
# http://www.minecraftforum.net/topic/1869433-noteblock-pitches-for-playsound/ <- by GCity306
...
inputs = (
		("NOTE BLOCK TO PLAYSOUND", "label"),
		("WARNING: This filter destroys the original Note Blocks"),
		("Sound to play:", ("string","value=mob.pig.say")),
	...
	#for iterY in xrange(box.miny,box.maxy): #Height
		...
		for iterZ in xrange(box.minz,box.maxz): #Depth
			for iterX in xrange(box.minx,box.maxx): #Width
				...
				if tbID == m2dID: # Only drop the selected material
					...
					if belowBlock == materialToReplace:
						...
						if belowBlock1 == 0 or belowBlock2 == 0 or belowBlock3 == 0 or belowBlock4 == 0:
							setBlock(level, tempBlock, iterX, iterY-1, iterZ)
