...
# v3 builds surfaces in blocks on poseable ArmorStands
		...
		for yy in xrange(-centreHeight, centreHeight):
			...
			for zz in xrange(-centreDepth, centreDepth):
				...
				if abs(a*x*x+b*y*y+c*z*z+2*f*y*z+2*g*z*x+2*h*x*y+2*p*x+2*q*y+2*r*z+d) <= TOLERANCE:
					...
					else: # Create an ArmorStand with the block in the head with the pose set up to follow the surface
						chunk = level.getChunk((centreWidth+xx)/CHUNKSIZE, (centreDepth+zz)/CHUNKSIZE)
						chunk.TileEntities.append( createArmorStand((int)(centreWidth+xx), (int)(centreHeight+yy), (int)(centreDepth+zz)))
