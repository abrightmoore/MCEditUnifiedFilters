...
def ParamSpheres(level, box, options):
	...
	while iterH <= 360:
		...
		while iterV <= 180:
			...
			theCommand = "/summon ArmorStand "+str(box.minx)+" "+str(box.miny)+" "+str(box.minz)+" {Pose:{Head:["+str(iterH)+"f,0.0f,"+str(iterV)+"f]},Equipment:[0:{},1:{},2:{},3:{},4:{id:redstone_block,Count:1b,Damage:0s}]}"
			
#			{CustomDisplayTile:1,DisplayTile:"+str(O_MATERIAL_ID)+",DisplayData:"+str(O_MATERIAL_DATA)+",DisplayOffset:"+str(O_RADIUS)+",Rotation:["+str(iterH)+"f,"+str(iterV)+"f]}"
