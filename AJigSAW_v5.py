...
inputs = (
		...
		("Operation",(
			...
			"Lattice",
			"Random Lines",
			"Random Non-Intersecting Lines",
  		    )),
		("Pen Block:", alphaMaterials.BlockofQuartz),
		("Seed:", 0),
		("Min Length:", 10),
		("Max Length:", 20),
		...
		for y in xrange(box.miny+int(height/7), box.maxy):
			if y%branchGap == branchGap-1: # at each point on the trunk just below the height of the branchGap
				...
				for i in xrange(0,branchNum):
					...
					drawBranch(level,matBark,matLeaf,(x0,y0,z0),sizeHere,theta,branchPhi,branchLength)
	...
	for branchLayer in branchLayers:
		for (x,y,z) in branchLayer:
			if chanceBauble <= R.randint(1,100):
				(mat,col) = matBauble
				col = R.randint(0,15)
				drawBauble(level,matBaubleFill,(mat,col),matString,(x,y-2,z),branchGap>>1,2) # Draw baubles
...
def drawBranch(level,matBark,matLeaf,(x0,y0,z0),startRadius,theta,phi,length):
...
#		for iter2 in xrange(0,spines):
	...
	if segmentSize > 5:
		...
		for iter in xrange(1,segments):
			...
			drawBranch(level,matBark,matLeaf,(x2,y2,z2),0,theta+segmentTheta,segmentPhi,((segments-iter)*segmentSize)>>1)
			drawBranch(level,matBark,matLeaf,(x2,y2,z2),0,theta-segmentTheta,segmentPhi,((segments-iter)*segmentSize)>>1)
