...
def Rotate(level, box, options):
	...
	c = (cx, cy, cz) = (0.0,0.0,0.0)  #(centreWidth,centreHeight,centreDepth)

	P = [ (0.0,0.0,0.0),
		  (width+1.0,0.0,0.0),
		  (width+1.0,0,depth+1.0),
		  (0.0,0.0,depth+1.0),
		  (0.0,height+1.0,0.0),
		  (width+1.0,height+1.0,0.0),
		  (width+1.0,height+1.0,depth+1.0),
		  (0.0,height+1.0,depth+1.0)
		]
	...
	for iterY in xrange(int(miny),int(maxy)):
		for iterZ in xrange(int(minz),int(maxz)):
			for iterX in xrange(int(minx),int(maxx)):
				# Plot the new point
				(ox,oy,oz) = UnRotatePoint((float(iterX),float(iterY),float(iterZ)), (cx,cy,cz), theta, phi)
...
#	newBox = BoundingBox( (box.minx+int(minx-centreOffsetX),box.miny+int(miny-centreOffsetY),box.minz+int(minz-centreOffsetZ)),(box.minx+int(maxx-centreOffsetX),box.miny+int(maxy-centreOffsetY),box.minz+int(maxz-centreOffsetZ)))
